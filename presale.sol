...
interface IPresaleGuita {
    function mint(address account_, uint256 amount_) external;
}

contract GuitaPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount BUSD deposited by user
        uint256 debt; // total GUITA claimed thus pGUITA debt
        bool claimed; // True if a user has claimed GUITA
    }

    address devAddress = 0x8d4c35112759b1931a8DD87DB26cC35FCbC01FbE;
    address public homeAddress = 0x1241F5eD4628E8999Fb5f3125a389868C5552184; 

    // Tokens to raise (BUSD) and for offer (pGUITA) which can be swapped for (GUITA)
    IERC20 public BUSD; // for user deposits
    IERC20 public pGUITA;
    IERC20 public GUITA;

    uint256 public price = 1 * 1e18; // 20 BUSD per pGUITA

    uint256 public cap = 10000 * 1e18; // 1000 BUSD cap per user

    uint256 public totalRaisedBUSD; // total BUSD raised by sale

    uint256 public totalDebt; // total pGUITA and thus GUITA owed to users
    ...
    bool public claimPresale; // true when pGUITA is claimable

    bool public contractPaused; // circuit breaker

    mapping(address => UserInfo) public userInfo;
    mapping(address => uint256) public guitaClaimable; // amount of GUITA claimable by address
    ...
    constructor(
        ...
        require( _pGUITA != address(0) );
        pGUITA = IERC20(_pGUITA);
        require( _GUITA != address(0) );
        GUITA = IERC20(_GUITA);
    ...
    /**
     *  @notice set GUITA Token
     *  @param _GUITA: address of GUITA
     */
    ...
    /**
     *  @notice set pGUITA Token
     *  @param _pGUITA: address of pGUITA
     */
    function setPGUITA(address _pGUITA) external onlyOwner {
        require( _pGUITA != address(0) );
        pGUITA = IERC20(_pGUITA);
    }

    /**
     *  @notice set price for pGUITA
     *  @param _price: price for pGUITA
     */
    ...
    // @notice lets users claim GUITA
    // @dev send sufficient GUITA before calling
    function claimUnlock() external onlyOwner {
        require(ended, "Sale has not ended");
        require(!claimable, "Claim has already been unlocked");
        require(GUITA.balanceOf(address(this)) >= totalDebt, 'not enough GUITA in contract');
        claimable = true;
        emit ClaimUnlocked(block.number);
    }


    // @notice lets users claim pGUITA
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 payout = _amount.mul(1e18).div(price).div(1e9); // pGUITA to mint for _amount

        totalDebt = totalDebt.add(payout);

        uint256 feeAmount = _amount.mul(10).div(100);
        BUSD.safeTransferFrom( msg.sender, devAddress, feeAmount );
        BUSD.safeTransferFrom( msg.sender, homeAddress, _amount.sub(feeAmount) );

        IPresaleGuita( address(pGUITA) ).mint( msg.sender, payout );

        emit Deposit(msg.sender, _amount);
    }
    
    /**
     *  @notice it deposits pGUITA to withdraw GUITA from the sale
     *  @param _amount: amount of pGUITA to deposit to sale (9 decimals)
     */
    function withdraw(uint256 _amount) external checkIfPaused {
        require(claimable, 'GUITA is not yet claimable');
        ...
        pGUITA.safeTransferFrom( msg.sender, address(this), _amount );

        GUITA.safeTransfer( msg.sender, _amount );

        emit Mint(address(pGUITA), msg.sender, _amount);
        emit Withdraw(address(GUITA), msg.sender, _amount);
    }
    ...
    // @notice it claims pGUITA back from the sale
    function claimPresaleGUITA() external checkIfPaused {
        require(claimPresale, 'pGUITA is not yet claimable');
        ...
        pGUITA.safeTransfer( msg.sender, payout );

        emit Withdraw(address(pGUITA),msg.sender, payout);
    }
