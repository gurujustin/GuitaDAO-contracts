...
contract MigrateDistributor is Policy {
    ...
    address public immutable sGuitaOld;
    
    address public newStaking;
    address public sGuitaNew;
    ...
    */
    ...
    constructor( address _treasury, address _guita, uint _epochLength, uint _nextEpochBlock, address _oldStaking, address _sGuitaOld, address _newStaking, address _sGuitaNew ) {
        ...
        require( _sGuitaOld != address(0) );
        sGuitaOld = _sGuitaOld;
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sGuitaNew != address(0) );
        sGuitaNew = _sGuitaNew;
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochBlock <= block.number ) {
            ...
            uint oldReward= split(reward,ISGUITA(sGuitaOld).circulatingSupply(),ISGUITA(sGuitaNew).circulatingSupply());
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( GUITA ).totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function setNewStaking(address _newStaking, address _sGuitaNew) external onlyPolicy() {
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sGuitaNew != address(0) );
        sGuitaNew = _sGuitaNew;
